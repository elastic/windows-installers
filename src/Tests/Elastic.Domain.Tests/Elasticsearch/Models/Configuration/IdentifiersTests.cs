...
namespace Elastic.Installer.Domain.Tests.Elasticsearch.Models.Configuration
{
	public class IdentifiersTests : InstallationModelTestBase
	{
		...
		[Fact]
		void ClusterNameNotEmpty() => this._model
		...
		[Fact]
		void NodeNameNotEmpty() => this._model
		...
		[Fact]
		void SeedsFromElasticsearchYaml() => WithExistingElasticsearchYaml($@"cluster.name: {_clusterName}
node.name: {_nodeName}
")
