...
namespace Elastic.Installer.Domain.Kibana.Model
{
		...
		: InstallationModelBase<KibanaInstallationModel, KibanaInstallationModelValidator>
	{
			...
			) : base(wixStateProvider, session, args)
		{
			...
			// TODO use real values here when ConfigurationModel is implemented x => x.ConfigurationModel.Host, x => x.ConfigurationModel.Port
			var observeHost = this.WhenAnyValue(x => x.ServiceModel.User, x => x.ServiceModel.Password,
